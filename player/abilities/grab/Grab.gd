...
func _physics_process(delta):
	...
	if (Input.is_key_pressed(KEY_E) or Input.is_joy_button_pressed(0, JOY_XBOX_Y)) and can_use:
		can_use = false
	...
	if not Input.is_key_pressed(KEY_E) and not Input.is_joy_button_pressed(0, JOY_XBOX_Y):
		can_use = true
